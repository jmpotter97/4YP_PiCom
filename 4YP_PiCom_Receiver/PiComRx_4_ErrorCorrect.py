...
def Remove_Padding(data_list, LOGS):
    ...
    if data_list[0] == 0 and data_list[-1] == 0:
        LOGS.append("Input fits padding form at least\n")
        del data_list[0]
        data_list.pop()
    else:
        LOGS.append("Input may have some issues\n")
...
if __name__ == '__main__':
    try:
        ...
        if GPIO.wait_for_edge(clock_pin, GPIO.RISING, timeout=10000) is not None:
            LOGS.append("Clock interrupt successful, receiver was started\n")
            Receive_Data(output, LOGS)
            if len(output) == 0:
                LOGS.append("No data was received\n")
            else:
                '''
                Remove_Padding(output, LOGS) ### WILL INCLUDE ONCE CONFIRMED TO WORK, MAYBE
